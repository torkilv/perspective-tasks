var Q = require('q');
var positionCalculator = require('./position-calculator');
var logger = require('../logger');
var Task = require('./task');

var NotFoundError = require('perspective-core-server').error.NotFoundError;
var ValidationError = require('perspective-core-server').error.ValidationError;
...
TasksService.prototype.allParents = function() {
  return this.taskRepository.allParents();
};
...
TasksService.prototype.insert = function(task, lastItemInListId) {
  ...
  if (task.hasParent()) {
    this.taskRepository.get(task.attr.parent).then(function(parent) {
      if (!parent) {
        deferred.reject(new NotFoundError("Could not find parent with id: " + task.attr.parent));
        return;
      }
