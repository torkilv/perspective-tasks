...
var setupTasks = function(serverAPI, dbAPI) {
  ...
  serverAPI.route('post', '/tasks', function(req, res, next) {
    ...
    var parentId = task.attributes.parentId;
    if (parentId) {
      delete task.attributes.parentId;

      hasWorkFlow(function() {
        if workflow
          task.attributes.status = workflow[0]

        dbAPI.insert(table, task.attributes, function(result) {

          dbAPI.update(table, parentId, result.id, function(result) {
            res.send(201, result);
            return next();
          });
