...
TasksService.prototype.insert = function(task, lastItemInListId) {
  ...
  var insert = function() {
    that.getPosition(lastItemInListId, null).then(function(position) {
      ...
      logger.debug("Inserting task", task.attributes);
...
TasksService.prototype.setPosition = function(task, previousId, nextId) {
  ...
  this.getPosition(previousId, nextId).then(function(position) {
    ...
    logger.debug("Updating position for task", task.attributes);
...
TasksService.prototype.getPosition = function(previousId, nextId) {
  ...
  if (previousId) {
    promises.push(this.taskRepository.getById(previousId))
  }

  if (nextId) {
    promises.push(this.taskRepository.getById(nextId));
  }
  ...
  Q.allSettled(promises).then(function(results) {
    ...
    results.forEach(function(result) {
      if (result.value.attributes.id === previousId) {
        previous = result.value;
      } else if (result.value.attributes.id === nextId) {
        next = result.value;
      }
    });
    ...
    if (!previous && !next) {
      ...
      position = positionCalculator.calculateLastItem(previous.attributes.position);
    } else if (!previous && next) {
      position = positionCalculator.calculate(next.attributes.position, null);
    } else {
      position = positionCalculator.calculate(next.attributes.position, previous.attributes.position);
    }
