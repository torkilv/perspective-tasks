...
TasksService.prototype.insert = function(task) {
  ...
  var insert = function() {
    that.getPosition(task.lastItemInListId, null).then(function(position) {
      ...
      console.log("Current: ", task.attributes.id, task.attributes.position);
...
TasksService.prototype.getPosition = function(previousId, nextId) {
  ...
  Q.all([
      ...
      console.log("Next: ",next.id, next.position);
      console.log("Previous: ", previous.id, previous.position);

      var position;
      if ((!previousId && !nextId) || previousId && !nextId) {
        position = positionCalculator.calculateLastItem(previous.position);
      } else {
        position = positionCalculator.calculate(next.position, previous.position);
      }
